...
class SafaricomController(http.Controller):
    ...
    def mpesa_stk_callback(self, payload):
        try:
            ...
            if not payment_method:
                return request.make_json_response({"ResultCode": "1", "ResultDesc": "Payment method not found"})

            # 🔥 FIX 1: Extract the Till ID from the verified hash
            pos_config_id = decoded_payload.get('pos_config_id')

            stk_callback = data.get('Body', {}).get('stkCallback', {})

            # 🔥 FIX 2: Pass the pos_config_id to the method
            payment_method._notify_stk_callback(stk_callback, pos_config_id=pos_config_id)

            return request.make_json_response({"ResultCode": "0", "ResultDesc": "Accepted"})

        except Exception as e:
            return request.make_json_response({"ResultCode": "1", "ResultDesc": str(e)})
    ...
    def c2b_confirmation_callback(self, payload):
        try:
            ...
            if not payment_method:
                return request.make_json_response({"ResultCode": "C2B00011", "ResultDesc": "Payment method not found"})

            if data:
                # For C2B, we keep it 'open' so the cashier can claim it manually
            ...
            return request.make_json_response({"ResultCode": "C2B00011", "ResultDesc": str(e)})